<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Users {{ __(request()->input('archived') == 'true' ? '(Archived)' : '(Active) ') }}
        </h2>
    </x-slot>
    ...
    <div class="overflow-x-auto p-6">
        ...
        <div class="flex justify-end gap-4">
            ...
            @if (request()->input('archived') == 'true')
                <a href="{{ route('users.index', ['archived' => 'false']) }}"
            ...
            @else
                <!-- Show "View Archived users" only when archived=false -->
                <a href="{{ route('users.index', ['archived' => 'true']) }}"
                    class="inline-block mb-4 bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-600">
                    🗃️ View Archived users
                </a>
            @endif
        ...
        <table class="min-w-full divide-y divide-gray-200 rounded-lg shadow mt-4 bg-white">
            <thead>
                <tr>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-grey-600"> Name</th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-grey-600"> Email </th>
                    <th class="px-6 py-3 text-left text-sm font-semibold text-grey-600"> Role </th>
            ...
            <tbody>
                @forelse ($users as $user)
                    <tr class=" border-b">
                        <td class="px-6 py-3 text-grey-800"> {{ $user->name }} </td>
                        <td class="px-6 py-3 text-grey-800"> {{ $user->email }} </td>
                        <td class="px-6 py-3 text-grey-800"> {{ $user->role }} </td>
